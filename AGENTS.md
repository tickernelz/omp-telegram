...
- A manual follower's owner identity names one terminal session: under a shared multiplexer server (`tmux`, `screen`) it carries an `@pane<n>`/`@screen<name>` scope so unrelated panes never collapse into one follower profile, and liveness parsing strips that suffix. The claimed letter is the slot authority: a registration commits its Workspace binding with the claim's slot, re-aligns the reused record to it, and may adopt a record on its claimed binding's exact target when that record is its own or its instance is dead.
- A Workspace binding is held only by a live process. A record whose `instanceId` names a provably absent process no longer blocks its own directory's claim, so the next session reclaims that binding, letter, and name instead of drifting to a new ordinal; unparseable or unprovable identities stay live. A Workspace-claimed provision never reuses a record bound to another Workspace binding, and a refused binding commit names its exact conflict rather than one opaque sentence.
