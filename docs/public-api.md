...
- `/telegram-host [install|uninstall|restart|status|attach] [--dry-run]` — manage a resident host: one systemd **user** unit that keeps a normal OMP session running in tmux, so Telegram stays served with no terminal open. Installation requires Linux with `systemctl` and `tmux`, writes `~/.config/systemd/user/omp-telegram-host.service` plus a wrapper under `<agentDir>/telegram-host/` and a `host.json` anchor in `<agentDir>`, then enables the unit with `Restart=always`. The host follows the command contract instead of spawning hidden Pi processes: `install` renders exactly two unit artifacts and enables the unit, `uninstall` disables it before removing artifacts and the private tmux socket, `restart` re-renders and restarts, `status` reports unit state and pane identity, and `attach` prints the exact `tmux -S <socket> attach -t host` command so the operator can watch or take over the terminal. `--dry-run` prints the full plan, every path, and all steps without touching the system, and the action plus `--dry-run` are offered as tab autocompletions. The rendered unit writes `WorkingDirectory=` as a literal path because systemd parses path settings verbatim, and escapes `%` as `%%` in every expanded value. Only the host session itself auto-runs `/telegram-connect` on session start, and only while it owns neither the lock nor a follower registration, which is the cold-start case the ordinary auto-start path cannot cover. That session is identified by the wrapper's `OMP_TELEGRAM_HOST` stamp or by membership in the host's private tmux socket; the agent-wide anchor marks an installation and never makes an ordinary terminal auto-connect. The unit runs a full interactive OMP session in a real PTY, so `/plan`, provider approvals, and the PTY bash overlay stay available; no OMP run mode is responsible for the host.
...
- `/telegram-commands [sync|reset]` — synchronize or reset Telegram bot slash commands and restore the menu button. Without arguments or with `sync`, registers OMP commands across both `default` and `all_private_chats` scopes (ensuring private chats reflect OMP commands even if previously configured by another bot framework) and sets `setChatMenuButton` to `{ type: "commands" }`. With `reset`, deletes existing scopes via `deleteMyCommands` before re-registering clean commands.
- `/telegram-settings [key] [value]` — open an interactive terminal TUI menu (`SettingsList`) to adjust bridge runtime preferences (display mode, progress tail detail, update interval, drafts, voice reply mode, time injection, thread cleanup). Also supports non-interactive CLI updates (e.g. `/telegram-settings interval 5000ms` or `/telegram-settings mode names`) and tab autocompletions.
...
- `ask({ questions })` is registered by this package under the name `ask`, which replaces OMP's builtin ask tool for the session. Each question carries `id`, `question`, optional `header`, `options[{ label, description?, preview? }]`, optional `multi`, and an optional `recommended` index that appends " (Recommended)". When both surfaces are live the tool races them: the OMP TUI dialog runs through the host's own ask implementation while Telegram renders the same question as inline buttons. The first surface to answer wins, the losing surface is aborted, and the losing Telegram message is edited to read `Answered: in the CLI`. The tool result names the winner — `Answered via Telegram.` or `Answered via CLI.` — and records `details.answeredVia`. Single-select answers on one tap; `multi` toggles checkbox-marked options behind a `Done selecting (n)` button; every question also gets an `Other (type your own)` button that clears the keyboard and consumes the next non-command text message from that exact chat/thread as free-text input. The tool blocks while a live surface can still answer and has no answer timeout; only the host's abort signal cancels it. Telegram is armed only when this session actually holds transport authority (direct lock ownership or a registered follower binding), so a disconnected bridge never leaves the tool waiting on a question nobody can receive. With no Telegram destination, or with a Telegram destination whose transport is down, the tool delegates to the native ask surface alone and the local dialog stays answerable; with no surface left at all it returns an error result telling the model to choose a conservative default rather than hanging.
...
- Bot API command sync publishes commands to both `default` and `all_private_chats` scopes, and sets the chat menu button to `{ type: "commands" }` so private chats always reflect OMP commands even when previously configured by external bot runtimes. Command sync executes automatically during leader polling startup, on `/telegram-connect`, on Telegram `/start` or `/help`, and via `/telegram-commands [sync|reset]`.
