...
- `/telegram-host [install|uninstall|restart|status|attach] [--dry-run]` — manage a resident host: one systemd **user** unit that keeps a normal OMP session running in tmux, so Telegram stays served with no terminal open. Installation requires Linux with `systemctl` and `tmux`, writes `~/.config/systemd/user/omp-telegram-host.service` plus a wrapper and `host.json` anchor under `<agentDir>/telegram-host/`, then enables the unit with `Restart=always`. The host follows the command contract instead of spawning hidden Pi processes: `install` renders exactly two unit artifacts and enables the unit, `uninstall` disables it before removing artifacts and the private tmux socket, `restart` re-renders and restarts, `status` reports unit state and pane identity, and `attach` prints the exact `tmux -S <socket> attach -t host` command so the operator can watch or take over the terminal. `--dry-run` prints the full plan, every path, and all steps without touching the system. A hosted agent directory auto-runs `/telegram-connect` on session start only while it owns neither the lock nor a follower registration, which is the cold-start case the ordinary auto-start path cannot cover. The unit runs a full interactive OMP session in a real PTY, so `/plan`, provider approvals, and the PTY bash overlay stay available; no OMP run mode is responsible for the host.
